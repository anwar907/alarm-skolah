...
class RingtoneService : Service() {
    ...
    private fun fireNotification() {
        ...
        var notification: Notification = NotificationCompat.Builder(this)
            .setContentTitle("Alarm is ging off")
    ...
    private fun playAlarm() {
        var alarmUrl: Uri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_ALARM)

        if (alarmUrl==null){
            alarmUrl = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION)
        }
        r = RingtoneManager.getRingtone(baseContext, alarmUrl)
        r.play()
    }
